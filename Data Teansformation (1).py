...
# 
# Scaling is essential because most machine learning algorithms calculate the mathematical distance between data points. If one feature has a large range (like Salary: 50,000) and another has a small range (like Age: 25), the algorithm will be biased toward the larger numbers. Scaling brings all features to a similar level, ensuring that every variable contributes equally to the model's predictions.

# Q10. Why do we convert categorical data into numerical form?
# 
# Machine Learning models are essentially mathematical equations that perform operations on matrices. These algorithms cannot process or "read" text labels like "Remote," "HR," or "Male." By converting categorical data into numbers (using techniques like One-Hot or Label Encoding), we enable the model to perform the necessary calculations and identify patterns within the data.
